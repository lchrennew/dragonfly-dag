...
const edgeObjToId = (isDirected, edgeObj) => edgeArgsToId(isDirected, edgeObj.v, edgeObj.w, edgeObj.name);
...
export default class Graph {
    ...
    sources() {
        const self = this;
        return _.filter(this.nodes(), v => _.isEmpty(self._in[v]));
    }

    sinks() {
        const self = this;
        return _.filter(this.nodes(), v => _.isEmpty(self._out[v]));
    }
    ...
    removeNode(v) {
        ...
        if (_.has(this._nodes, v)) {
            ...
            if (this._isCompound) {
                ...
                _.each(this.children(v), child => self.setParent(child));
    ...
    filterNodes(filter) {
        ...
        if (this._isCompound) {
            _.each(copy.nodes(), v => copy.setParent(v, findParent(v)));
        }
